...
const [writeFile, readFile, exec] = [fs.writeFile, fs.readFile, childProcess.exec].map(fun => util.promisify(fun));
...
if (package) {
  ...
  const links = {
    vscode: "https://marketplace.visualstudio.com/items?itemName=sudoaugustin.tailwindcss-transpiler",
    atom: "https://atom.io/packages/tailwinidcss-transpiler",
  };
  const name = names[editor];
  const link = links[editor];
  const replace = markDown => markDown.replace(/\@name/, name).replace(/\@link/, link);
  readFile("./readme.md")
